	...
	public class XTRACE
        {
                ...
                private static int ColorToCOLORREF(System.Windows.Media.Color color) 
                { 
                        return ((color.R | (color.G << 8)) | (color.B << 0x10)) & 0xffffff;
                }

                public static void trace(System.Windows.Media.Color c, string format, params object[] args)
                {
                        dllLoad();
                        if (null == hPorc)
                                return;
	        ...
	        public static void trace(string format, params object[] args)
	        {
                        ...
                        if (null == hPorc)
                                return;
                        int value = ColorToCOLORREF(System.Windows.Media.Color.FromRgb(0, 0, 0));
                        DateTime dt = DateTime.Now;
                        string tm1 = string.Format("{0:yyyy-MM-dd HH:mm:ss: }", dt);
                        hPorc(1, value, tm1, 0, 0);
                        hPorc(1, value, string.Format(format, args), 0, 0);
		...
		{
			trace(Color.FromRgb(0, 0, 0), buf, len);
		}

		public static void trace(System.Windows.Media.Color c, byte[] buf, int len)
		{
